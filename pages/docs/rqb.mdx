...
<Callout>
  Drizzle performs partial selects on the query level, no additional data is
  transfered from the database
</Callout>
...
<Callout type="info" emoji="ℹ️">
  When both `true` and `false` select options are present, all `false` options
  are ignored
</Callout>
...
<Callout type="warning" emoji="⚠️">
  As of now aggregations are not supported in `extras`, please use [`core
  queries`](/docs/crud) for that
</Callout>
